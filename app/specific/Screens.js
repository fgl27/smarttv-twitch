...
function Screens_addFocusVideo(y, x, idArray, forceScroll) {
    if ((y < 2 && Main_YchangeAddFocus(y)) || forceScroll) {
        ...
        if (!y) Main_ScrollTable(idArray[10], screen.height * 0.035);
        ...
        if (y > 1) {
            if (Main_ThumbNull((y + 1), 0, idArray[0])) {
                Main_ScrollTable(idArray[10],
                    (document.getElementById(idArray[0] + y + '_' + x).offsetTop * -1) + (screen.height * 0.21));
            } else Main_handleKeyUp();
        } else Main_ScrollTable(idArray[10], screen.height * 0.035);
    ...
    if (Main_YchangeAddFocus(y) || forceScroll) {
        ...
        if (inUseObj.posY < (inUseObj.Cells.length - 1) || forceScroll) {
            Main_ScrollTable((idArray[10] ? idArray[10] : idArray[7]),
                (document.getElementById(idArray[5] + y + '_' + x).offsetTop * -1) + screen.height * 0.0125);
        }

    } else if ((inUseObj.Cells.length - 1) === y && (Main_ThumbNull(y - 1, x, idArray[0]))) {

        Main_ScrollTable((idArray[10] ? idArray[10] : idArray[7]),
            (document.getElementById(idArray[5] + (y - 1) + '_' + x).offsetTop * -1) + screen.height * 0.0125);
    } else Main_handleKeyUp();
}
